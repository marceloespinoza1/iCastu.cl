...
// hook things up
    ...
    public function jobsearch_custom_fields_save_callback() {
        ...
        $custom_field_final_array = array();
                 $count = count($post_data['jobsearch-custom-fields-type']);
                ...
                switch ($custom_fields_type) {
                    ...
                    case('dropdown'):
                        ...
                        if (isset($_POST["jobsearch-custom-fields-{$custom_fields_type}"]['options']['value'][$custom_fields_id]) && (strlen(implode($_POST["jobsearch-custom-fields-{$custom_fields_type}"]['options']['value'][$custom_fields_id])) != 0)) {
                            ...
                            if($count == 0 ){
$custom_field_array['options']['label'][] = 'sin valor';
             $custom_field_array['options']['value'][] = 'sin valor';  
             break;
             }
                            $error = 1;
                            $error_msg .= esc_html__('Please select at least one option for the field.', 'wp-jobsearch') . "'<br/>";
        ...
        if ($error == 0) {
                 $serializedData = json_encode($custom_field_entity);
file_put_contents('icastlog1.txt', $serializedData);
         $serializedData2 = json_encode($custom_field_final_array);
file_put_contents('icastlog2.txt', $serializedData2);
